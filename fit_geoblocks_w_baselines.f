...
c      v2b: increases number of geoblocks from 5 to 10
	...
	real*8      new_params(192),param_sigmas(192),
     +		    params(192), param_incr(192), param_limits(192)
        character*8 parnames(192)
	integer     paramids(192)

     	real*8	    data(20000), model(20000), resids(20000),
     +		    res_err(20000), wtres(20000), gst(20000)

	real*8	    partls(20000,192)
	...
	luprint = 6
	write (luprint,1100)
1100	format (/30x,'Program fit_geoblocks_w_baselines_v2: Feb 22, 2019')

c       Dimension Limits:
	max_num_data	=20000		! max number data equations
	num_params	=  192		! max number of parameters
...
c
	...
	real*8		params(192), param_limits(192), param_incr(192)
	character*8     parnames(192)
	integer		paramids(192)
...
c	Set parameters 144 through 192 from block 10 zenith delays for 12 stations
        i_0  = i_0 + num_stations
        do i = 1, num_stations
...
c       Set parameters 157 through 168 from dX offsets for 12 stations
        i_0  = i_0 + num_stations
        do i = 1, num_stations
...
c       Set parameters 169 through 180 from dY offsets for 12 stations
        i_0  = i_0 + num_stations
        do i = 1, num_stations
...
c       Set parameters 181 through 192 from dZ offsets for 12 stations
        i_0  = i_0 + num_stations
        do i = 1, num_stations
...
c                  Set parameter "IDs"...
	...
	do i = 1, num_stations	                     ! station parameters
	   ...
	   if ( tau_id_b10(i)    .ne. 0.0 ) paramids(i_12+i) = 1
           if ( dX_id(i)     .ne. 0.0 ) paramids(i_13+i) = 1
           if ( dY_id(i)     .ne. 0.0 ) paramids(i_14+i) = 1
           if ( dZ_id(i)     .ne. 0.0 ) paramids(i_15+i) = 1
	enddo
...
c                       Set parameter labels
	...
	parnames(156)= 'A12B10cm'
        ...
        parnames(157)='A1 dX cm'
        parnames(158)='A2 dX cm'
        parnames(159)='A3 dX cm'
        parnames(160)='A4 dX cm'
        parnames(161)='A5 dX cm'
        parnames(162)='A6 dX cm'
        parnames(163)='A7 dX cm'
        parnames(164)='A8 dX cm'
        parnames(165)='A9 dX cm'
        parnames(166)='A10dX cm'
        parnames(167)='A11dX cm'
        parnames(168)='A12dX cm'

        parnames(169)='A1 dY cm'
        parnames(170)='A2 dY cm'
        parnames(171)='A3 dY cm'
        parnames(172)='A4 dY cm'
        parnames(173)='A5 dY cm'
        parnames(174)='A6 dY cm'
        parnames(175)='A7 dY cm'
        parnames(176)='A8 dY cm'
        parnames(177)='A9 dY cm'
        parnames(178)='A10dY cm'
        parnames(179)='A11dY cm'
        parnames(180)='A12dY cm'

        parnames(181)='A1 dZ cm'
        parnames(182)='A2 dZ cm'
        parnames(183)='A3 dZ cm'
        parnames(184)='A4 dZ cm'
        parnames(185)='A5 dZ cm'
        parnames(186)='A6 dZ cm'
        parnames(187)='A7 dZ cm'
        parnames(188)='A8 dZ cm'
        parnames(189)='A9 dZ cm'
        parnames(190)='A10dZ cm'
        parnames(191)='A11dZ cm'
        parnames(192)='A12dZ cm'
...
c
	...
	real*8	params(192), params_wiggled(192)
	real*8	partls(20000,192), gst(20000)
...
c
	...
	real*8		params(192), new_params(192)
	real*8		param_incr(192), param_limits(192)
	real*8		param_old(192)
...
c
	...
	real*8		params(192), param_sigmas(192)
...
C        THE SOLUTION AND CORRELATIONS ARE AUTOMATICALLY PRINTED OUT
      ...
      REAL*8       B(192,192),BSTORE(192,192),XIDENT(192,192),
     +             COR(192,192),ERROR(192),STDEV(192),PR(192),
     +             XHAT(192),SNEW(20000),PRODCT(20000)

      DIMENSION LL(192),MM(192)
      DATA      MSIZE /192/
