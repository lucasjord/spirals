...
# Manual phasecal
#
def geo_man_pcal(indata, refant, logfile, geo_src=None, geo_tr=None):
    fringe          = AIPSTask('FRING')
    fringe.indata   = indata
    fringe.refant   = refant
    fringe.docal    = 1
    ...
    if geo_src==None and geo_tr==None:
        (source,timerange)=get_best_scan(indata_x, logfile, 'geoqual.dat', 1)
    else:
        (source,timerange) = geo_src,geo_tr
...
# Manual phasecal
#
def geo_man_pcal_sx(indata_s, indata_x, refant, logfile, geo_src=None, geo_tr=None):
    fringe          = AIPSTask('FRING')
...
def get_geosource_stats(indata):
    ...
    snr=[]
    for i in arange(0,len(sources)+1):
        snr.append([])
...
def plot_baseline(indata,ant, inter_flag, ptype, doplot_flag, logfile):
    ...
    draw()
    if ptype=='A':
        if os.path.exists('./delay-rate.ps'): os.remove('./delay-rate.ps')
...
def plotatmos(inter_flag, logfile):
    ...
    for i in arange(int(max(data[:,0]))):
        ant.append([])
        data2.append([])
        avg.append(-1)
        rms.append(-1)
...
def run_wa_pang(indata,refant,cl_in=3,cl_out=4):
    ...
    with open('wa_pang.TBIN','w+') as w:
        for row in header: print(row, file=w)
        for j in arange(len(t)):
            for i in arange(nif):
                if i==0: 
                    print('{0:8.0f}{1:>24.15E}{2:>15.6E}{3:>11.0f}{4:>11.0f}{5:>11.0f}{6:>11.0f}{7:>15.3f}{8:>11.0f}{9:>15.3f}{10:>15.3f}{11:>15.3f}{12:>15.6f}{13:>15.6f}{14:>15.3f}{15:>15.3f}{16:>15.3f}{17:>11.0f}{9:>15.3f}{10:>15.3f}{11:>15.3f}{18:>15.6f}{19:>15.6f}{14:>15.3f}{15:>15.3f}{16:>15.3f}{17:>11.0f}'.format(
                j+1, t[j],dt[j]/(24*3600.),sid[j],4,1,0,0,0,0,0,0,cos( feed_angle[j]),sin( feed_angle[j]),0,0,1,refant,cos(-feed_angle[j]),sin(-feed_angle[j])),file=w) 
                else: 
                    print('{0:8.0f}{1:>24s}{1:>15s}{1:>11s}{1:>11s}{1:>11s}{1:>11s}{1:>15s}{1:>11s}{1:>15s}{1:>15s}{1:>15s}{2:>15.6f}{3:>15.6f}{4:>15.3f}{5:>15.3f}{6:>15.3f}{7:>11.0f}{1:>15s}{1:>15s}{1:>15s}{8:>15.6f}{9:>15.6f}{4:>15.3f}{5:>15.3f}{6:>15.3f}{7:>11.0f}'.format(
                j+1,"''",cos( feed_angle[j]),sin( feed_angle[j]),0,0,1,refant,cos(-feed_angle[j]),sin(-feed_angle[j])),file=w) 
        print('***END*PASS***',file=w)
...
def check_calsource(indata, calsource):
    ...
    if sour=='n' or sour=='N':
        ...
        for i in arange(len(real_sources)):
...
def make_check_RDBE(data,logfile,inter_flag,dtype):
    ...
    if len(antennas)<max(antennas):
        for i in arange(1,max(antennas)):
...
def run_snplt_2(indata, inver, optype, snplt_name):
    ...
    for n in arange(1,indata.table_highver('AIPS PL')+1):
...
def get_download_names(ou, op, of):
    ...
    if inter_flag==1 and of[0]==0:
        ...
        if cont=='n' or cont=='N':
            order=[]
            for i in arange(0,len(file_names)):
                entry = file_names[i]
...
def make_multiviewcontrol(indata,in2data,centre,mv_window=30.,outfile='multiview/multiview_control.inp'):
    if os.path.exists(outfile): os.remove(outfile)
    #making su tables
    su_cal = [[s.id__no, s.source, s.raepo, s.decepo] for s in indata.table('SU',1)]
    su_cnt = [[s.id__no, s.source, s.raepo, s.decepo] for s in in2data.table('SU',1) if centre in s.source][0]
    with open(outfile,'w+') as w:
        print( '! {} QSO offsets from target source {}'.format(outname[0][:-2],su_cnt[1].strip(' ')),file=w) 
        print('! ',file=w) 
        print('! Number of IFs in multiview.TBOUT file',file=w) 
        print('   {}'.format(indata.header['naxis'][3]),file=w) 
        print('! ',file=w) 
        print('! Time window (min) multiview fitting (eg, 15 => +/-15 min from a target time)',file=w) 
        print('  {}'.format(mv_window),file=w) 
        print('! Following source numbers from LISTR(scan)...NEEDS TO BE CORRECT',file=w) 
        print('! Src#   Name           dX     dY  in degrees',file=w) 
        for cal in su_cal:
            print(' {:2.0f}     {:<10s}    {:>5.2f}  {:>5.2f}'.format(cal[0],cal[1].strip(' '),
                                                                      (cal[2]-su_cnt[2])*np.cos(su_cnt[3]*np.pi/180.),
                                                                      cal[3]-su_cnt[3]),file=w) 
...
if 'geo_src' in locals() and globals(): pass
else: geo_src = None
if 'geo_tr' in locals() and globals(): pass
else: geo_tr = None
...
try:
    ...
    sys.exit('Did not find file, did you run LOGIN.SH?')
mprint('##################################',logfile)
...
if geo_fringe_flag==1 and do_geo_block==1 and sx_geo==False:
    ...
    if refant_flag==1: 
        refant=select_refant(geo_data)
    testfringe(geo_data, refant, 0, logfile)
    if refant_flag==1: 
        refant=select_refant2(geo_data,logfile)
    geo_man_pcal(geo_data, refant, logfile, geo_src, geo_tr)
    ...
    if delzn_flag==1:
        ...
        if sx_geo==False:
            ...
            if os.path.exists('tropos'):
                os.system('rm -rf tropos')
            os.mkdir('tropos')
            os.system('rm -rf fit_geodetic_bl*')
            os.system(fit_path+'fit_geoblocks_tropos > fit_geoblocks.prt')
            ...
            runatmos(geo_data, atmos_file, cl_in=4, cl_out=5)
        else:
            os.chdir('geoblock')
            if os.path.exists('tropos'):
...
if not antname == 'LBA':
    ...
    #   pass
    # check ALL line_data sources in cont_data
    src_array = np.array([(s in cont_data.sources) for s in line_data.sources])
    #print(src_array)
    if np.all(src_array):
        pass
    else:
...
if co_fringe_flag==1 and line==cont:
    ...
    # commented out by LJH on 5/11/21
    # when would you need cvelled continuum data??
        ...
        for nant in arange(len(ants)):
            ...
            for i in arange(len(rtime)):
                ...
                if len(M)>2:
                    ...
                    r_phas[r_phas < -np.pi] +=  2*np.pi
                    for j in arange(len(src_num[indx])):
                        count1 = count1 + 1
...
if imv_prep_flag==1:
    ...
    if line_data2.exists() and line!=cont:
        ...
        calsource = findcal(cont_data,calsource)
        # make split file for calsource
        run_split(cont_data, [calsource], split_outcl, doband, bpver)
        multi = AIPSTask('multi') # not for multiview, coincidental naming and use
    ...
    if line!=cont:
        runtbout(linedata, 'SN',5,'./multiview/target.TBOUT')
    elif line==cont:
        runtbout(cal_split,'SN',1,'./multiview/target.TBOUT')
    ...
    if not os.path.exists('multiview/phase_unwrapper.inp'):
        with open('multiview/phase_unwrapper.inp','w+') as f:
            for ant in list(get_ant(cont_data).keys()):
                print(ant, end=' ', file=f)
                for s in np.zeros(shape=len(target)): print(s, end=' ', file=f)
                print(file=f)
...
if imultiv_flag==1:
    ...
    for n in ants.keys():
        ...
        to the centre of the track, where it is most unlikely that a PWA is occurring. Since there can also
        ...
        mprint('Unwrapping '+str(N)+' multiview blocks for antenna '+ants[n], logfile)
        ...
        p2 = np.copy(p)
        for nb in arange(N-1):
            if nb==0:
                # go down from first mbv
                for j in arange(Nquas):
                    for i in arange(p[:int(nboundaryindx[0]),j].shape[0]-1):
                ...
                for j in arange(Nquas):
                    for i in arange(p[int(1+nboundaryindx[nb-1]):int(nboundaryindx[nb]),j].shape[0]-1):
        ...
        for j in arange(Nquas):
                for i in arange(p[int(1+nboundaryindx[nb]):,j].shape[0]-1):
        ...
        if not os.path.exists('multiview/block_{}_unwrapper.inp'.format(ants[n])):
            with open('multiview/block_{}_unwrapper.inp'.format(ants[n]),'w+') as f:
                for block in arange(N):
                    print(block+1, end=' ', file=f)
                    for s in np.zeros(shape=len(target)): print(s, end=' ', file=f)
                    print(file=f)
        ## open up file and load in user defined offsets. If file was just created, will only contain zeroes
        blocks_uwraps  = splitt(get_file('multiview/block_{}_unwrapper.inp'.format(ants[n])))[:,1:].astype(float)
        if not blocks_uwraps.shape==(N,Nquas):
            sys.exit('Something wrong with block_unwrapper logic for antenna '+ants[n])
        # apply offsets to each multiview block
        for nb in arange(N):
            if nb==0:
                correction[:int(nboundaryindx[nb]),:] = correction[:int(nboundaryindx[nb]),:] + 360*blocks_uwraps[nb,:]
            elif nb==N-1:
                correction[int(nboundaryindx[nb-1]):,:] = correction[int(nboundaryindx[nb-1]):,:] + 360*blocks_uwraps[nb,:]
            else:
                correction[int(nboundaryindx[nb-1]):int(nboundaryindx[nb])] = correction[int(nboundaryindx[nb-1]):int(nboundaryindx[nb])]+ 360*blocks_uwraps[nb,:]
...
if imv_imagr_flag==1:
    ...
    if line!=cont:
        ...
        linedata.clrstat()
        check_sncl(linedata,7,10,logfile)
        ...
        if cal_split.exists():
            ...
            runimagr(cal_split,calname,niter,cellsize,imsize,1,imna1,antennas,
                uvwtfn,robust,beam,baselines=ant_bls,timer=imgr_timer,gainu=2)
            _zapbeam(calsource[:11-len(imna1)]+imna1,inseq=1)
            runimagr(cal_split,calname,niter,cellsize,imsize,1,imna2,antennas,
                uvwtfn,robust,beam,baselines=ant_bls,timer=imgr_timer,gainu=3)
...
if phase_cal_flag==1:
    ...
    for i in arange(len(phase_loop)):
...
if amp_cal_flag==1:
    ...
    if len(dofit)!=len(amp_loop):
        ...
        for i in arange(len(dofit)):
            dofit[i]=0

    indata = AIPSUVData(source,split_outcl,line_data.disk,1)
    indata = check_cal(indata, source, outdisk, logfile, imna)
    for i in arange(len(amp_loop)):
