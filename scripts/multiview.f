...
c----------------------------
c     Combines phases of for N>=3 QSOs (Target-QSO position data) to estimate
...
c     parameters arenot used, but may be good diagnostics
...
c----------------------------
      ...
      real*8  target_times(1000), target_ants(1000)
      real*8  target_reals(1000), target_imags(1000)
      ...
      real*8  time_store(1000), phase_store(1000) 
      integer iant_store(1000)

      
c     Set some maximum control values, switches, and logical unit numbers...
      max_num_data = 1000                 ! integrations
...
c              print*,'DEBUG: target time: ',t_center, npts_QSO
...
c                    print*,'DEGUG: MV data: ',n,MV_src(n)
...
c              print*,'DEBUG: Total number of MV hits ',n_MV

c              Check that we have at least 3 MV QSOs to fit a plane
               call count_data( n_MV, n_QSOs, MV_src, 
     +                          n_hits_per_QSO, num_QSOs ) 
               if ( num_QSOs .ge. 3 ) then
...
c                    Need to check (again) for at least 3 good ones...
...
c===================================================================
     ...
     +                              reals, imags )
      ...
      real*8  times(1000), antennas(1000), reals(1000), imags(1000)
...
c     Document...
 ...
 1000          format(' MultiView QSO ',a13,' #:',i3,
     +                '; (X,Y) offset (deg)=',2f6.2)
            ...
            print*,' STOP: Need >=3 Multiview QSOs. Found ',n_QSOs
            stop
      endif
